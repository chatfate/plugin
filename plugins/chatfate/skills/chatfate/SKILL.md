...
7. 工具成功后，在内置浏览器打开本次工具返回的完整 `reportUrl`，有本轮内置交付标签就复用。外部 Google 登录标签不作为最终报告交付标签。最终回复固定为 `报告已生成：<reportUrl>`，完整链接逐字保留，不复述报告正文。
...
- 错误包含 `/login/start?ticket=…` 时，优先在内置浏览器打开该链接一次；只有明确不兼容时再使用外部浏览器。登录绑定当前服务端会话，无需刷新凭据。
